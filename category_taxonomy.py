...
MULTI_KEYWORD_RULES = [
    ...
    # broader existing rule (e.g. bare "pate"->Cold Cuts, bare "lamb brand"->
    # Herbs & Spices, bare "chocolate"->Chocolates) that would otherwise win first
    # under classify_by_name's first-match-wins Pass 0 loop. Verified against the
    # production CSV ground truth by a dedicated research agent.
    # ==========================================================================
    ("Cat", ['after dark', 'pate']),
    ("Fish & Other Animals", ['dog toy']),
    ("Cakes", ['tala', 'cake board']),
    ("Cat", ['pate', 'cat']),
    ("Cat", ['poesie']),
    ("International Cuisine", ['raspberry', 'vinegar']),
    ("Cheese", ['ricotta', 'limone']),
    ("Canned Vegetables", ['chicken', 'bean', 'sauce']),
    ("Spirits - Whisky", ['lone wolf', 'gin']),
    ("Hair Treatment", ['manuka', 'yogurt', 'cream']),
    ("Hair Treatment", ['rose', 'rice water']),
    ("Turkey", ['rafi', 'turkey', 'cranberry']),
    ("Gift Sets", ['florio', 'bottle bag']),
    ("Shower Gels", ['bodywash', 'jasmine']),
    ("Fish & Other Animals", ['trixie', 'stainless steel', 'bowl']),
    ("Cat", ['wet food', 'turkey', 'salmon']),
    ("International Cuisine", ['suntat', 'pizza', 'salami']),
    ("Shower Gels", ['baby bath foam']),
    ("Pizza", ['baguette', 'butter filling']),
    ("Chocolates", ['deco', 'chewing gum']),
    ("Butter", ['lurpak', 'spreadable olive']),
    ("Perfume", ['armaf', 'edp']),
    ("Chicken", ['turkey', 'emmenthal']),
    ("Vegetables", ['bulgar', 'vegetable']),
    ("Cake Preparations", ['lamb', 'gocciole']),
    ("International Cuisine", ['strak', 'pretzel']),
    ("Cheese", ['wensleydale', 'cranberries']),
    ("Wine - White", ['white wine', 'gift box']),
    ("Biscuits", ['biscuits for cheese']),
    ("Dog", ['dog', 'pate']),
    ("Food & Condiments", ['mini', 'lentil', 'crisp']),
    ("Disposables", ['vaschetta', 'alluminio']),
    ("Candles", ['citronella', 'alluminio']),
    ("International Cuisine", ['rye bread', 'croutons']),
    ("Easter", ['monadro', 'egg']),
    ("Biscuits", ['barbie', 'biscuits']),
    ("Legumes & Nuts", ['lamb', 'minestra']),
    ("International Cuisine", ['karums', 'curd']),
    ("Cake Preparations", ['lamb', 'cake', 'sponge']),
    ("Seasonal Items", ['auntys', 'steamed pudding']),
    ("Hair Treatment", ['olio', 'cream', 'ricci']),
    ...
    # ==========================================================================
    # Sixteenth pass (28 Aug 2026): large-scale collision-resolution push.
    # ~429 tier-0 promotions generated by 6 parallel research agents analyzing all
    # 478 non-accepted collision pairs from a fresh live categorize_listings.py run
    # (1134 colliding listings), each verified against the production CSV ground
    # truth. Per the standing directive to close collisions in bulk, not one at a
    # time. Some entries promote to a THIRD category (neither side of the reported
    # collision pair) when that's what the CSV ground truth actually shows.
    # ==========================================================================
    ("Vegetables", ['pastallegre', 'gnocchi']),
    ("Shower Gels", ['shower mousse']),
    ("Dilutables", ['sodastream', 'syrup']),
    ("Cakes", ['ballara', 'cinnamon']),
    ("Cakes", ['livity cakes']),
    ("Ironmongery", ['dekton', 'staple']),
    ("Ironmongery", ['dekton', 'scissors']),
    ("Ironmongery", ['dekton', 'ruler']),
    ("Household Goods", ['muffin pan']),
    ("Household Goods", ['cake pan']),
    ("Bread", ['bun sheet']),
    ("Biscuits", ['galbusera', 'riso']),
    ("Chocolates", ['riso scotti', 'cioccolato']),
    ("Canned Vegetables", ['waitrose essential', 'in water']),
    ("Vegetables", ["d'aucy", 'in water']),
    ("Vegetables", ['stir-fry vegetables', 'in water']),
    ("Butter", ['philadelphia', 'salmon']),
    ("Spirits - Vodka", ['vodka', 'raspberry']),
    ("International Cuisine", ['ajinomoto', 'oyakata']),
    ("Herbs & Spices", ['schwartz', 'lamb']),
    ("International Cuisine", ['buenas', 'vinegar']),
    ("Vinegars", ['biona', 'vinegar']),
    ("Toys & Games", ['on the wall', 'party hat']),
    ("Cake Preparations", ['budino', 'cioccolato']),
    ("Cake Preparations", ['cameo', 'mousse']),
    ("Yoghurt", ['mousse protein']),
    ("Herbs & Spices", ['mamma che brodo']),
    ("Intimate Care", ['durex', 'comfort']),
    ("Intimate Care", ['o.b.', 'comfort']),
    ("Household Goods", ['ad trend', 'bin']),
    ("Butter", ['margarine', 'sunflower oil']),
    ("Butter", ['butter & oil spray']),
    ("Butter", ['butter spd']),
    ("International Cuisine", ['tartufi']),
    ("Cheese", ['cheshire truffle']),
    ("Vegetables", ['crunchies', 'onion']),
    ("Vegetables", ['innocent plus', 'matcha']),
    ("Sports", ["lion's mane"]),
    ("Cake Preparations", ['dragon', 'ginger powder']),
    ("Canned Vegetables", ['antipasto', 'sun dried']),
    ("Cold Cuts", ['orto pausa', 'speck']),
    ("Sugar", ['dietorelle', 'sweetner']),
    ("Yoghurt", ['milk kefir', 'muesli']),
    ("Frozen Fish", ['beer', 'haddock']),
    ("Juices", ['succo', 'ace']),
    ("Honey", ['red currant jelly', 'port']),
    ("Pet Food", ['cliffi', 'canaries']),
    ("Laundry Washing Liquids", ['fabuloso', 'detersivo']),
    ("Laundry Washing Liquids", ['fabuloso', 'washing liquid']),
    ("Fabric Softener", ['comfort', 'strawberry']),
    ("Deodorants", ['salt of the earth', 'rollon']),
    ("International Cuisine", ['aceto di riso']),
    ("Fresh Pastry", ['w/oil']),
    ("Pasta & Couscous", ['scotti', 'astuccio']),
    ("Pork", ['belcando', 'chickpeas']),
    ("Fabric Softener", ['chicco', 'softner']),
    ("Fabric Softener", ['chicco', 'softener']),
    ("Snacks", ['patatine', 'caserecce']),
    ("Yoghurt", ['yofu', 'cranberry']),
    ("Perfume", ['so..? edp']),
    ("Spirits - Whisky", ['togouchi', 'beer']),
    ("Spirits - Whisky", ['grants', 'whisky']),
    ("Meat Alternatives", ['green vie', 'oregano']),
    ("Stationery", ['legami', 'port']),
    ("Household Goods", ['tekmee', 'port']),
    ("Vegetables", ['soyavit', 'peppers']),
    ("Baby Hygiene", ['octopus', 'bath toy']),
    ("Seasonal Items", ['intex', 'fish']),
    ("Ironmongery", ['dekton', 'water pump']),
    ("Ironmongery", ['dekton gloves', 'orange']),
    ("Gift Vouchers", ['so…?', 'gift set']),
    ("Men Care", ['old spice', 'clear gel']),
    ("Herbs & Spices", ['carmencita', 'water']),
    ("Air Fresheners", ['febreze', 'lenor']),
    ("Chocolates", ['kinder bueno', 'eggs']),
    ("Chocolates", ['maltesers', 'eggs']),
    ("Baby Essentials", ['chicco', 'perfumed water']),
    ("Soups", ['zuppa', 'fagioli']),
    ("Soups", ['zuppa', 'ceci']),
    ("Hair & Nail Accessories", ['the barb', 'nail scissors']),
    ("Hair & Nail Accessories", ['the barb', 'hair brush']),
    ("Juices", ['succo', 'zucchero']),
    ("All-purpose Cleaners", ['anticalcare', 'aceto']),
    ("Canned Seafood", ['tonno', 'piselli']),
    ("Canned Seafood", ['tonno', 'fagioli']),
    ("International Cuisine", ['svinska']),
    ("International Cuisine", ['slavonska']),
    ("Fabric Softener", ['unstoppables', 'dash']),
    ("Fabric Softener", ['cenndie', 'softner']),
    ("Baby Essentials", ['chicco', 'gums rubbing']),
    ("Flour", ['dragon chickpea flour']),
    ("Women Care", ['nivea', 'body cream', 'new']),
    ("Biscuits", ['terre di puglia', 'taralli']),
    ("Biscuits", ['deco', 'schiacciatine']),
    ("Wine - Red", ['tamari', 'malbec']),
    ("Biscuits", ['rayner', 'cookies']),
    ("Juices", ['innocent', 'juicy water']),
    ("Skin Care", ['face facts', 'lip balm']),
    ("Household Goods", ['pedrini', 'nut']),
    ("Gift Sets", ['whisky stones']),
    ("Frozen", ['haagen dazs', 'coffee']),
    ("Snacks", ['palse', 'crunchy']),
    ("Disposables", ['ipak', 'toothpick']),
    ("Sports", ['football scarf']),
    ("Laundry Washing Liquids", ['liquid detergent', 'aloe vera']),
    ("Biscuits", ['pasta mandorla']),
    ("Chocolates", ['pasta di mandorla']),
    ("International Cuisine", ['suntat']),
    ("Ham", ['tacchino', 'ham']),
    ("Cereal & Cereal Bars", ['lenny', 'marshmallow']),
    ("Herbs & Spices", ['turkey', 'seasoning']),
    ("Ironmongery", ['dekton', 'spanner']),
    ("Hair Treatment", ['pony tail']),
    ("Stationery", ['mitama']),
    ("Sauces & Condiments", ['chicken tonight', 'cream']),
    ("Summer", ['noodle chair']),
    ("Seasonal Items", ['pool noodle']),
    ("Ironmongery", ['dekton', 'multipurpose rope']),
    ("Ironmongery", ['dekton', 'no more nails']),
    ("Baby Food", ['aptamil milk']),
    ("Frozen Vegetables", ['orogel', 'burger']),
    ("Household Goods", ['deco', 'bobina']),
    ("Skin Care", ['chupa chups', 'lip balm']),
    ("International Cuisine", ['al ameed']),
    ("Fish & Other Animals", ['animology']),
    ("Legumes & Nuts", ['pellito', 'olives']),
    ("Frozen Vegetables", ['bonduelle', 'frozen', 'bolognese']),
    ("Cakes", ['melegatti']),
    ("Household Goods", ['electric water kettle']),
    ("Appliances", ['water kettle']),
    ("Cake Snacks", ['barilla', 'flauti']),
    ("Hair Treatment", ['keratin dream long cream']),
    ("Deodorants", ['deo', 'cucumber']),
    ("Disposables", ['stewo', 'napkins']),
    ("Wine - Red", ['uno', 'primitivo']),
    ("Wine - Red", ['mandorla', 'primitivo']),
    ("Wine - Red", ['mandorla', 'syrah']),
    ("Sauces & Condiments", ['panna', 'salmone']),
    ("Cat", ['tuna', 'whitemeat']),
    ("Frozen", ['warburtons', 'toaster']),
    ("All-purpose Cleaners", ['ace', 'bleach']),
    ("Cheese", ['cranberry', 'cheese']),
    ("Sauces & Condiments", ['schwartz', 'casserole']),
    ("Cheese", ['santa maria', 'cheese']),
    ("Cheese", ['kimifinne', 'cheese']),
    ("Floor Cleaners", ['pet pro', 'broom']),
    ("Household Goods", ['vileda', 'glitzi', 'always']),
    ("Sauces & Condiments", ['truffle', 'mustard']),
    ("Herbs & Spices", ['ricola', 'herbs']),
    ("Snacks", ['chewing', 'gum', 'cinnamon']),
    ("Spirits - Vodka", ['brewdog', 'vodka']),
    ("Disposables", ['eco planet', 'toilet roll']),
    ("Disposables", ['eco planet', 'kitchen roll']),
    ("Hair & Nail Accessories", ['hello kitty', 'puff']),
    ("Baby Essentials", ['hello kitty', 'glove']),
    ("Dish Washing Liquid", ['fairy', 'dishwash', 'spray']),
    ("Shower Gels", ['bodywash', 'oil']),
    ("Shower Gels", ['showel gel', 'oil']),
    ("Juices", ['rauch', 'multivitamin']),
    ("Juices", ['amigo', 'multivitamin']),
    ("Chocolates", ['cuorenero', 'astuccio']),
    ("Stationery", ['brunnen', 'eraser']),
    ("Skin Care", ['hello kitty', 'face mask']),
    ("Skin Care", ['take care', 'bath bomb']),
    ("Household Goods", ['sant anna', 'sparkling', 'water bottle']),
    ("Water", ['bormioli', 'globo', 'water glass']),
    ("Cooking Creams", ['president', 'pasta cream']),
    ("Air Fresheners", ['chupa chups', 'air freshner']),
    ("Rice", ['truffle', 'risotto']),
    ("Eggs", ['tala', 'egg slicer']),
    ("Eggs", ['tefal', 'egg slicer']),
    ("Milk", ['jacklon', 'milk']),
    ("Shower Gels", ['bodywash', 'goat milk']),
    ("Chilled Fish", ['emy', 'shrimp']),
    ("Chilled Fish", ['sword fish']),
    ("Cereals", ['nature valley', 'cereal']),
    ("Dilutables", ['robinsons', 'squash']),
    ("Insect Killer", ['squitos']),
    ("Eggs", ['relkon', 'pony']),
    ("Eggs", ['relkon', 'patrol']),
    ("Honey", ['darlington', 'jelly']),
    ("Deodorants", ['salt of the earth', 'refill', 'melon']),
    ("Household Goods", ['westmark', 'corkscrew']),
    ("Ironmongery", ['dekton', 'dash cam']),
    ("Spirits - Whisky", ['jameson', 'coffee']),
    ("Wine - White", ['frutino', 'merlot']),
    ("Pasta & Couscous", ['ferrero', 'polenta']),
    ("Make Up", ['lip smacker', 'ice cream']),
    ("Shower Gels", ['vidal', 'foam bath']),
    ("Legumes & Nuts", ['lamb', 'beans']),
    ("Cat", ['tuna', 'ham', 'broth']),
    ("Nappies", ['diaper', 'muslin']),
    ("Sanitary Towels", ['nuvenia', 'pad']),
    ("Seasonal Items", ['bestway', 'air hammer']),
    ("Hair Colouring", ['wella', 'koleston']),
    ("Shower Gels", ['jacklon', 'bath cream']),
    ("Stationery", ['depesche', 'diary']),
    ("Hair Treatment", ['got2b']),
    ("Conditioners", ['ultra dolce', 'olio', 'avocado']),
    ("Stationery", ['legami', 'usb hub']),
    ("Meat Alternatives", ['asiago food', 'veggie']),
    ("Chocolates", ['valsoia', 'cioccolato']),
    ("Olive Oil", ['carrefour', 'riso', 'olio']),
    ("Body Lotions", ['hand', 'body cream', 'olive oil']),
    ("Milk", ['bono', 'milk', 'croissant']),
    ("Chocolates", ['vidal', 'cola bottles']),
    ("Intimate Care", ['carefree', 'intimate wash']),
    ("Shampoos", ['tresemme', 'coconut']),
    ("Chilled Fish", ['razor clams']),
    ("Skin Care", ['clinians', 'tanning water']),
    ("Honey", ['mackays', 'preserve']),
    ("Household Goods", ['martini', 'sponge']),
    ("Pasta & Couscous", ['riso scotti', 'si con riso']),
    ("Sausages", ['bennati', 'salsiccia']),
    ("Spirits - Whisky", ['teacher', 'whisky']),
    ("Skin Care", ['chupa chups', 'hand cream']),
    ("Honey", ['tala', 'honey dipper']),
    ("Rice", ['basmati', 'bag']),
    ("Men Care", ['nivea men', 'all purpose']),
    ("Crackers, Crispbread & Breadsticks", ['bocconcini croccanti']),
    ("Nuts", ['balsamic', 'peanut']),
    ("All-purpose Cleaners", ['disinfectant', 'crisp linen']),
    ("Olive Oil", ['basil', 'evoo']),
    ("Hair Treatment", ['got2b', 'salt']),
    ("Skin Care", ['eye pads', 'cucumber']),
    ("Legumes", ['cranberry', 'bean']),
    ("Wine - Red", ['negro amaro']),
    ("Baby Essentials", ['chicco', 'bodywash']),
    ("Cloths & Sponges", ['ushuaia', 'sponge']),
    ("Dried Fruit", ['rayner', 'raisin']),
    ("Chips", ['handcooked', 'vinegar']),
    ("Spirits - Liqueurs", ['liqueur', 'pepper']),
    ("Dishwasher Tablets", ['powerball', 'uno']),
    ("Dog", ['cesar', 'piselli']),
    ("Dog", ['junior chicken', 'salmon']),
    ("Spirits - Whisky", ['johnnie walker', 'gift box']),
    ("Stock Cubes", ['stock pot', 'wine']),
    ("Chocolates", ['kinder', 'easter']),
    ("Air Fresheners", ['febreze', 'cookie']),
    ("Toys & Games", ['intex', 'sprinkle']),
    ("Fabric Softener", ['ammorbidente', 'olio']),
    ("Pet Care", ['pet remedy', 'diffuser refill']),
    ("Stationery", ['william lamb', 'backpack']),
    ("Vegetables", ['cucumber', 'brine']),
    ("Pasta & Couscous", ['vermicelli', 'charcoal']),
    ("Toothbrushes", ['sensodyne', 'spazzolino']),
    ("Skin Care", ['clinians', 'milk']),
    ("Hair & Nail Accessories", ['franck provost', 'brush']),
    ("Hand Tools", ['jld', 'plier']),
    ("Pork", ['retro pork', 'frankfurter']),
    ("Wine - Red", ['maynards', 'port']),
    ("Clothes", ['peanuts', 'sock']),
    ("Baby Essentials", ['chicco', 'absorbent']),
    ("Honey", ['honey', 'truffle']),
    ("Biscuits", ['arluy', 'teddy']),
    ("Cold Cuts Prepacked", ['whisky bbq', 'ribeye']),
    ("Disposables", ['aloe vera', 'toilet roll']),
    ("Dog", ['miglior cane', 'mousse']),
    ("Disposables", ['napkin', 'champagne']),
    ("Sugar", ['dolcificante', 'astuccio']),
    ("Water", ['highland spring', 'water']),
    ("Crackers, Crispbread & Breadsticks", ['lentil', 'crispbread']),
    ("Frozen", ['frozen', 'cornflake']),
    ("Ciders", ['crisp apple']),
    ("First Aid", ['scholl', 'cream']),
    ("Drain Unblockers", ['disgorgante', 'candeggina']),
    ("Herbs & Spices", ['beer', 'chicken', 'rub']),
    ("Cake Preparations", ['crocker', 'milk']),
    ("Shampoos", ['alpecin', 'energizer']),
    ("Coffee", ['coffee', 'chicory']),
    ("Nuts", ['serano', 'beer']),
    ("Spirits - Liqueurs", ['hagerty', 'grappa']),
    ("Make Up", ['lip balm', 'gift box']),
    ("Dental Care", ['gum', 'interdental']),
    ("Air Fresheners", ['glade', 'cinnamon']),
    ("Honey", ['maple syrup']),
    ("Cake Preparations", ['bankok', 'desiccated']),
    ("Household Goods", ['barrettine', 'torch oil']),
    ("Household Goods", ['estia', 'kettle']),
    ("Frozen Vegetables", ['frozen', 'galette']),
    ("Air Fresheners", ['airwick', 'crisp linen']),
    ("Skin Care", ['lycia', 'depilatory']),
    ("Cereals", ['granola', 'oil']),
    ("Wine - Rose", ['lachryma vitis', 'rose']),
    ("Cake Preparations", ['steamed pudding']),
    ("Skin Care", ['aveeno', 'bodywash']),
    ("Disposables", ['toothpick', 'decorative']),
    ("Yoghurt", ['fage', 'honey']),
    ("Crackers, Crispbread & Breadsticks", ['cranberry', 'crispbread']),
    ("Olive Oil", ['truffle', 'evoo']),
    ("Adult Nappies", ['tena', 'blanc']),
    ("Pasta & Couscous", ['girasoli', 'salsiccia']),
    ("Cereals", ['barbie', 'cereal']),
    ("Perfume", ['edp', 'gin']),
    ("Dental Care", ['interdental', 'orange']),
    ("Pasta & Couscous", ['pasta', 'always']),
    ("Dilutables", ['tala', 'cordial']),
    ("Clothes", ['socks', 'mustard']),
    ("Cat", ['purina', 'dualnature']),
    ("Juices", ['innocent', 'matcha']),
    ("Cold Cuts", ['fuet', 'truffle']),
    ("Stationery", ['willam', 'barbie']),
    ("Pasta & Couscous", ['ravioli', 'barolo']),
    ("Yoghurt", ['ehrmann', 'hazelnut']),
    ("Olives", ['henaff', 'olive']),
    ("Skin Care", ['simple', 'hydrating']),
    ("Insect Killer", ['citronella', 'windlight']),
    ("Snacks", ['cheez it']),
    ("Cakes", ['riso scotti', 'cornetto']),
    ("Body Lotions", ['foot', 'heel', 'cream']),
    ("Dog", ['biscotti', 'notte']),
    ("Frozen", ['arancini', 'pancetta']),
    ("Cakes", ['foil cake']),
    ("Frozen Vegetables", ['frozen', 'garden pea']),
    ("Biscuits", ['mulino bianco', 'fette biscottate']),
    ("Household Goods", ['calzanetto', 'cream']),
    ("Household Goods", ['korbond', 'scissors']),
    ("Air Fresheners", ['airwick', 'gel tin']),
    ("Dish Washing Liquid", ['cenndie', 'piatti']),
    ("Shampoos", ['elvive', 'multivitamin']),
    ("Body Lotions", ['hand cream', 'lip balm']),
    ("Sauces & Condiments", ['chef', 'salmon']),
    ("Laundry Washing Powders", ['non bio', 'powder']),
    ("Honey", ['honey', 'comb']),
    ("Coffee", ['lavazza', 'soave']),
    ("Drain Unblockers", ['households', 'unblocker']),
    ("Skin Care", ['panno', 'struccante']),
    ("Cheese", ['cheddar', 'port']),
    ("Canned Vegetables", ['chickpea', 'drained']),
    ("Sanitary Towels", ['carefree', 'aloe vera']),
    ("Fish & Other Animals", ['trixie', 'catnip']),
    ("Spirits - Liqueurs", ['honey', 'liqueur']),
    ("Frozen", ['gelati', 'granola']),
    ("Butter", ['truffle', 'butter']),
    ("Cold Cuts", ['salame', 'campagnolo']),
    ("Chicken", ['chicken', 'flaxseed']),
    ("Pasta & Couscous", ['truffle', 'pasta']),
    ("Snacks", ['twistees', 'cream']),
    ("Disposables", ['tissues', 'balsamic']),
    ("Household Goods", ['depesche', 'mug']),
    ("Bread", ['baguette', 'frozen']),
    ("Sausages", ['frankfurter', 'frozen']),
    ("Canned Vegetables", ['divella', 'peperoncino']),
    ("Ironmongery", ['dekton', 'washer']),
    ("Bathroom & Wc Cleaner", ['kettle', 'descaler']),
    ("Bathroom & Wc Cleaner", ['harpic', 'orange']),
    ("Household Goods", ['led golf ball']),
    ("All-purpose Cleaners", ['sgrassatore', 'ginger']),
    ("Household Goods", ['applied nutrition', 'water bottle']),
    ("Sugar", ['dragon', 'xylitol']),
    ("Crackers, Crispbread & Breadsticks", ['grissini', 'olio']),
    ("International Cuisine", ['tapioca', 'round paper']),
    ("Cloths & Sponges", ['pearly', 'sponge']),
    ("Disposables", ['cotoneve', '160pcs']),
    ("Cooking Creams", ['condisoia', 'cooking cream']),
    ("Sports", ['protein bar']),
    ("Sauces & Condiments", ['sugo', 'vongole']),
    ("Cakes", ['dan cake', 'rum']),
    ("Gift Sets", ['beer glass', 'bell']),
    ("Air Fresheners", ['moth', 'freshener']),
    ("Air Fresheners", ['airflor', 'talco']),
    ("Juices", ['cranberry', 'nectar']),
    ("Butter", ['whiskey', 'butter']),
    ("Seasonal Items", ['intex', 'pump']),
    ("Wipes", ['chamomile', 'wipes']),
    ("Pasta & Couscous", ['pasta', 'olive']),
    ("Rice", ['risotto', 'speck']),
    ("Household Goods", ['gin glass']),
    ("Skin Care", ['sheet mask', 'ice cream']),
    ("Floor Cleaners", ['swiffer', 'honey']),
    ("Household Goods", ['bottle', 'frozen']),
    ("Sanitary Towels", ['absorbents', 'comfort']),
    ("Meat Alternatives", ['valsoia', 'budino']),
    ("Meat Alternatives", ['valsoia', 'pudding']),
    ("Wine - White", ['frutino', 'chardonnay']),
    ("Gift Sets", ['gift set', 'bath']),
    ("Cake Preparations", ['rayner', 'qaghaq']),
    ("Shower Gels", ['showel gel']),
    ("Men Care", ['proraso', 'barba']),
    ("Frozen", ['ice cream', 'cookies']),
    ("Floor Cleaners", ['mop', 'zigzag']),
    ("Frozen Fish", ['frozen', 'vongole']),
    ("Cooking Creams", ['cream', 'diary']),
    ("Sausages", ['salsiccia', 'rosmarino']),
    ("Skin Care", ['intimate cleanser']),
    ("Beef", ['milk fed', 'veal']),
    ("Party Food", ['balloons', 'helium']),
    ("Skin Care", ['nivea men', 'body cream']),
    ("Household Goods", ['scrub a duck']),
    ("Dog", ['w', '125g']),
    ("Cake Preparations", ['rayner', 'coffee']),
    ("Dog", ['pedigree', 'jelly']),
    ("Disposables", ['greaseproof', 'muffin']),
    ("Biscuits", ['valsoia', 'cookie']),
    ("Chilled Fish", ['salmon', 'multigrain']),
    ("Nuts", ['almond', 'speculaas']),
    ("Coffee", ['pocket coffee', '18p']),
    ("Skin Care", ['derma v10', 'cucumber']),
    ("Spirits - Whisky", ['wild turkey']),
    ("Spirits - Whisky", ['tokaj']),
    ("Spirits - Whisky", ['fireball', 'whisky']),
    ("Ironmongery", ['oil can']),
    ("Cake Preparations", ['lamb', 'yeast']),
    ("Wine - Rose", ['rosado']),
    ("Frozen", ['ice cream', 'kit kat']),
    ("Hair Treatment", ['hair mask']),
    ("Chilled Fish", ['ajinomoto', 'prawn']),
    ("All-purpose Cleaners", ['pronto', 'detergente']),
    ("Crackers, Crispbread & Breadsticks", ['crispbread', 'sea salt']),
    ("Household Goods", ['fish bone', 'tweezers']),
    ("Pasta & Couscous", ['cake', 'ravioli']),
    ("Electrical", ['babyliss', 'comb']),
    ("Turkey", ['turkey', 'stuffed']),
    ("Skin Care", ['derma v10', 'foot pack']),
    ("Laundry Washing Liquids", ['eco egg']),
    ("Laundry Washing Liquids", ['ecoegg']),
    ("Chocolates", ['valor', 'caco']),
    ("Hand Wash Liquids", ['sapone liquido']),
    ("Fabric Softener", ['comfort heavenly nectar']),
    ("Toys & Games", ['disney', 'scrunchie']),
    ("Vegetables", ['torta', 'leeks']),
    ("Disposables", ['aluminium tray']),
    ("Dilutables", ['bolero', 'multivitamin']),
    ("Turkey", ['fig', 'turkey']),
    ("Wine - Red", ['red wine', 'vinegar']),
    ("Herbs & Spices", ['chorizo', 'paprika']),
